"""Boston Dynamics — bostondynamics.com/careers.

bostondynamics.com/careers links straight out to a Workday tenant,
`bostondynamics.wd1.myworkdayjobs.com`, site `Boston_Dynamics` (found by
grepping the careers page HTML for `myworkdayjobs.com`, then confirmed live
with curl). Same public, unauthenticated CXS JSON API used elsewhere in this
project (boeing.py, draper.py, anl.py, bnl.py, fnal.py, ames.py):

    POST https://bostondynamics.wd1.myworkdayjobs.com/wday/cxs/bostondynamics/Boston_Dynamics/jobs
         {"appliedFacets": {}, "limit": ..., "offset": 0, "searchText": "..."}

    GET  https://bostondynamics.wd1.myworkdayjobs.com/wday/cxs/bostondynamics/Boston_Dynamics{externalPath}
         (externalPath from the search response, e.g.
         "/job/Waltham-MA/Staff-Full-Stack-Software-Engineer_R2879") — used
         here for a real job description to build the snippet, since the
         search response itself carries no description text.

Confirmed live (2026-08-26): "robot" returned 76 real matches (Staff Fleet
Operations Robot Captain - Atlas, Robotics Operations Technician IV, Senior
Staff UX Designer - Human Robot Interaction, etc., mostly Waltham MA /
Billerica MA / Mountain View).
...
endpoint's `timeType` (e.g. "Full time"), same as boeing.py/draper.py.
...
TENANT_URL = "https://bostondynamics.wd1.myworkdayjobs.com"
SITE_ID = "Boston_Dynamics"
SEARCH_URL = f"{TENANT_URL}/wday/cxs/bostondynamics/{SITE_ID}/jobs"
DETAIL_URL_BASE = f"{TENANT_URL}/wday/cxs/bostondynamics/{SITE_ID}"
...
def search_boston_dynamics(keyword: str, location: str = None, radius_miles: int = None) -> list[dict]:
    """`location`/`radius_miles` are accepted for interface parity with the
    other search_* functions but unused — Boston Dynamics' Workday tenant
...
if __name__ == "__main__":
    jobs = search_boston_dynamics("robot")
