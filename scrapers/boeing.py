"""The Boeing Company — jobs.boeing.com / boeing.wd1.myworkdayjobs.com.

jobs.boeing.com is a Workday "career site" custom domain — the page itself
is served from boeing.com's own infra, but its login link and (per browser
network activity) its job search both point at a real Workday tenant:
`boeing.wd1.myworkdayjobs.com`, site `EXTERNAL_CAREERS` (found by grepping
the jobs.boeing.com homepage HTML for `myworkdayjobs.com`, then confirmed
live with curl).

Hitting the custom domain's own `/wday/cxs/...` path 302-redirects (doesn't
serve JSON directly) — confirmed live 2026-08-26. The tenant's own domain
serves the same public, unauthenticated CXS JSON API used elsewhere in this
project (anl.py, bnl.py, fnal.py, nrel.py, ames.py):

    POST https://boeing.wd1.myworkdayjobs.com/wday/cxs/boeing/EXTERNAL_CAREERS/jobs
         {"appliedFacets": {}, "limit": ..., "offset": 0, "searchText": "..."}

    GET  https://boeing.wd1.myworkdayjobs.com/wday/cxs/boeing/EXTERNAL_CAREERS{externalPath}
         (externalPath from the search response, e.g.
         "/job/USA---Tukwila-WA/E-7A-Systems-Engineer..._JR2026517550-2") —
         used here for a real job description to build the snippet, since
         the search response itself carries no description text.

Confirmed live (2026-08-26): 690 total postings on the tenant; "engineer"
returned 579 matches (E-7A Systems Engineer, Software Engineer, Systems
Engineer, etc.); "physicist" returned 0 — a real, verified zero (Boeing's
postings apparently don't use that exact word), not a broken query.
...
endpoint (matching what a human sees on the page), so `salary` is always
None.
...
`offset` up to a `_MAX_RESULTS` safety cap, since Boeing's result volumes
can run into the hundreds for a broad keyword. This tenant also only
...
TENANT_URL = "https://boeing.wd1.myworkdayjobs.com"
SITE_ID = "EXTERNAL_CAREERS"
SEARCH_URL = f"{TENANT_URL}/wday/cxs/boeing/{SITE_ID}/jobs"
DETAIL_URL_BASE = f"{TENANT_URL}/wday/cxs/boeing/{SITE_ID}"
...
def search_boeing(keyword: str, location: str = None, radius_miles: int = None) -> list[dict]:
    """`location`/`radius_miles` are accepted for interface parity with the
    other search_* functions but unused — Boeing's Workday tenant search is
